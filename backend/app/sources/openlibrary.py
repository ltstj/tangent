...
def fetch_subject(subject: str, limit: int = 25) -> list[CatalogItem]:
    ...
    params = {"subject": subject, "limit": limit, "fields": "key,title,first_publish_year,subject,ratings_average,want_to_read_count"}
...
def search(query: str, limit: int = 10) -> list[CatalogItem]:
    with httpx.Client(timeout=30) as client:
        data = client.get(f"{BASE}/search.json", params={"q": query, "limit": limit,
            "fields": "key,title,first_publish_year,subject,ratings_average"}).json()
